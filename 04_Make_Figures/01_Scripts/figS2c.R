...
order_of_env_factors <- vdata_its[which(vdata_its$microbe == "Chitinolytic bacteria in soil"),]
...
# Order the microbes
vdata_its$microbe <- factor(vdata_its$microbe, levels = c("Methanotrophic bacteria in leaves", 
                                                          ...
                                                          "Chitinolytic bacteria in soil"))

# Plot figure s2c
figS2d <- ggplot(vdata_its) +
...
figS2d
ggsave(paste0("atherton_figures2d_functionalguilds_vs_environment_heatmap_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), plot = figS2d, 
       device = cairo_pdf, width = 10, height = 5, units = "in", dpi = 300)
