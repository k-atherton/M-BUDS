...
# Plot Figure 2a
fig2a <- ggplot(vdata, aes(x = site_edge, color = guild)) +
  ...
  geom_text(data = vdata, family = "Arial",
            ...
            show.legend = FALSE) +
  theme_classic() +
  labs(y = "Fungal animal pathogens\nrelative abundance (%)", x = "", color = "", 
       fill = "") +
  ...
  scale_color_manual(values = categorical_pal[c(3)], guide = "none") +
  scale_fill_manual(values = categorical_pal[c(3)], guide = "none") +
  ggtitle("Fungal animal pathogens in leaves")
