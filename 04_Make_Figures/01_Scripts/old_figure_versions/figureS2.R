...
# Keep only the percent organic matter
...
# Format the name of the nutrient
vdata$Nutrient <- c(rep("Soil organic matter",5))
...
vdata$tukey <- c("b", # urban street trees
                 "a", # urban forest edge
                 "a", # urban forest interior
                 "a", # rural forest edge
                 "a" # rural forest interior
)
...
figs2a <- ggplot() +
  ...
  # This adds the tukey group labels
  ...
  geom_point(data = metadata_soil_16s,
             aes(x = site_edge, y = soil_percent_organic_matter * 100, 
                 color = site_edge),
...
figs2a
ggsave(paste0("atherton_figures2a_organicmattersoil_vs_treetype_", 
