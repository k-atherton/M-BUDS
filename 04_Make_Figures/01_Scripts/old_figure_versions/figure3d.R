...
fig3d <- ggplot(metadata_leaf_16s, aes(x = site_edge, y = perc_methanotroph, 
  ...
  geom_jitter(aes(stroke = NA, color = site_edge), 
              size = 0.75,  alpha = 0.2) +
  ...
  geom_errorbar(data = summary_data,
                aes(x = site_edge, y = mean, ymin = mean - sd, ymax = mean + sd,
                    group = site_edge, color = site_edge),
  ...
  geom_crossbar(data = summary_data,
                aes(x = site_edge, y = mean, ymin = mean - se, ymax = mean + se,
                    fill = site_edge, color = site_edge),
...
ggsave(paste0("atherton_figure3d_methanotrophicleaves_vs_treetype_", 
       ...
       plot = fig3d, dpi = 300, width = 5.75, height = 3.75, units = "in")
