...
pca_leaf_df_its$tree_age <- factor(pca_leaf_df_its$tree_age, levels = c("Y", "M", "O"))

print(adonis2(leaf_aitch_its~pca_leaf_df_its$tree_age))

# Plot figure s3a
figs3a_dbh <- ggplot(pca_leaf_df_its, 
                         aes(x = PC1, y = PC2, color = tree_age)) +
...
figs3a_dbh
ggsave(paste0("atherton_figures7a_ITSpcaleaves_DBH_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3a_dbh, dpi = 300, width = 6, height = 3.5, units = "in")

### STATISTICS: TREE GENOTYPE ###
print(adonis2(leaf_aitch_its~pca_leaf_df_its$tree_species))

# Plot figure s3a
figs3a_sp <- ggplot(pca_leaf_df_its, 
                     aes(x = PC1, y = PC2, color = tree_species)) +
  geom_point(shape = 18, alpha = 0.5, size = 2.5) +
  xlab(paste("Principal Component 1\n(", 
             round(pca_variance_leaf_its[1] * 100, 2), "% variance)", 
             sep = "")) +
  ...
  theme(text = element_text(family = "Arial"),
        ...
        plot.title = element_text(size = 10), legend.position = "bottom") +
  ggtitle("Fungal community in leaves") + 
  scale_color_manual(values = c(categorical_pal, linear_pal))

figs3a_sp
ggsave(paste0("atherton_figures7e_ITSpcaleaves_sp_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3a_sp, dpi = 300, width = 6, height = 3.5, units = "in")
...
pca_leaf_df_16s$tree_age <- factor(pca_leaf_df_16s$tree_age, levels = c("Y", "M", "O"))

print(adonis2(leaf_aitch_16s~pca_leaf_df_16s$tree_age))

# Plot figure s3b
figs3b_dbh <- ggplot(pca_leaf_df_16s, 
                       aes(x = PC1, y = PC2, color = tree_age)) +
  geom_point(shape = 18, alpha = 0.5, size = 2.5) +
  xlab(paste("Principal Component 1\n(", 
...
figs3b_dbh
ggsave(paste0("atherton_figures7b_16Spcaleaves_DBH_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3b_dbh, dpi = 300, width = 6, height = 3.5, units = "in")

### STATISTICS: TREE GENOTYPE ###
print(adonis2(leaf_aitch_16s~pca_leaf_df_16s$tree_species))

# Plot figure s3b
figs3b_sp <- ggplot(pca_leaf_df_16s, 
                     aes(x = PC1, y = PC2, color = tree_species)) +
  geom_point(shape = 18, alpha = 0.5, size = 2.5) +
  xlab(paste("Principal Component 1\n(", 
             round(pca_variance_leaf_16s[1] * 100, 2), "% variance)", sep = "")) +
...
figs3b_sp
ggsave(paste0("atherton_figures7f_16Spcaleaves_species_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3b_sp, dpi = 300, width = 6, height = 3.5, units = "in")
...
# define the spatial matrix
dec.degrees.its_soil <- as.matrix(cbind(pca_soil_df_its$tree_longitude, 
                                 pca_soil_df_its$tree_latitude))
rownames(dec.degrees.its_soil) <- pca_soil_df_its$Sample 

# find the spatial distance between samples
space.matrix_its_soil <-rdist.earth(dec.degrees.its_soil[,1:2], miles=FALSE)

# define the design matrix
design_its_soil <- as.data.frame(matrix(nrow = nrow(pca_soil_df_its), 
                                         ncol = 5, data = 0))
rownames(design_its_soil) <- pca_soil_df_its$Sample
colnames(design_its_soil) <- c("street_tree", "urban_edge", "urban_interior", 
                          "rural_edge", "rural_interior")
for(i in 1:nrow(design_its_soil)){
  tree_type <- pca_soil_df_its$tree_pit_type[which(pca_soil_df_its$Sample == rownames(design_its_soil)[i])]
  if(tree_type == "Street Tree"){
    design_its_soil$street_tree[i] <- 1
  } else if(tree_type == "Urban Edge"){
    design_its_soil$urban_edge[i] <- 1
  } else if(tree_type == "Urban Interior"){
    design_its_soil$urban_interior[i] <- 1
  } else if(tree_type == "Rural Edge"){
    design_its_soil$rural_edge[i] <- 1
  } else if(tree_type == "Rural Interior"){
    design_its_soil$rural_interior[i] <- 1
  }
}

# Find the design distance between samples
design.dist_its_soil <- dist(design_its_soil)

# Test the effect of space and design on the PCA groupings
MRM_soil <- lm(as.numeric(as.dist(soil_aitch_its)) ~  as.numeric(design.dist_its_soil) + as.numeric(as.dist(space.matrix_its_soil)))
anova(MRM_soil)

pca_soil_df_its$tree_age <- "O"
pca_soil_df_its$tree_age[which(pca_soil_df_its$tree_dbh_2021 < 30)] <- "M"
pca_soil_df_its$tree_age[which(pca_soil_df_its$tree_dbh_2021 < 10)] <- "Y"

pca_soil_df_its$tree_age <- factor(pca_soil_df_its$tree_age, levels = c("Y", "M", "O"))

print(adonis2(soil_aitch_its~pca_soil_df_its$tree_age))

# Plot figure s3c
figs3c_dbh <- ggplot(pca_soil_df_its, 
                 aes(x = PC1, y = PC2, color = tree_age)) +
...
figs3c_dbh
ggsave(paste0("atherton_figures3c_ITSpcabelowground_DBH_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3c_dbh, dpi = 300, width = 6, height = 3.5, units = "in")

### STATISTICS: TREE GENOTYPE ###
print(adonis2(soil_aitch_its~pca_soil_df_its$tree_species))

# Plot figure s3c
figs3c_sp <- ggplot(pca_soil_df_its, 
                     aes(x = PC1, y = PC2, color = tree_species)) +
  geom_point(aes(shape = sample_type), alpha = 0.5, size = 2) +
  xlab(paste("Principal Component 1 \n(", 
             round(pca_variance_soil_its[1] * 100, 2), "% variance)", sep = "")) +
  ...
  theme(text = element_text(family = "Arial"),
        ...
        plot.title = element_text(size = 10), legend.position = "none") +
  ggtitle("Fungal community belowground") + 
  scale_color_manual(values = c(categorical_pal, linear_pal), guide = "none") + 
  scale_shape_manual(values = shapes)

figs3c_sp
ggsave(paste0("atherton_figures3g_ITSpcabelowground_species_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3c_sp, dpi = 300, width = 6, height = 3.5, units = "in")
...
pca_soil_df_16s$tree_age <- factor(pca_soil_df_16s$tree_age, levels = c("Y", "M", "O"))

print(adonis2(soil_aitch_16s~pca_soil_df_16s$tree_age))

# Plot figure s3D
figs3d_dbh <- ggplot(pca_soil_df_16s, 
                       aes(x = PC1, y = PC2, color = tree_age)) +
  geom_point(aes(shape = sample_type), alpha = 0.5, size = 2) +
  xlab(paste("Principal Component 1 \n(", 
...
figs3d_dbh
ggsave(paste0("atherton_figures7d_16Spcabelowground_DBH_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3d_dbh, dpi = 300, width = 6, height = 3.5, units = "in")
...
# Plot figure s3D
figs3d_sp <- ggplot(pca_soil_df_16s, 
                     aes(x = PC1, y = PC2, color = tree_species)) +
  geom_point(aes(shape = sample_type), alpha = 0.5, size = 2) +
  xlab(paste("Principal Component 1 \n(", 
             round(pca_variance_soil_16s[1] * 100, 2), "% variance)", sep = "")) +
...
figs3d_sp
ggsave(paste0("atherton_figures7h_16Spcabelowground_species_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs3d_sp, dpi = 300, width = 6, height = 3.5, units = "in")
