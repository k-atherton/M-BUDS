...
# Plot figure 4c
fig4c <- ggplot(diversity_long, aes(x = log(tree_dist_to_boston_km +1), 
                                    y = value, color = diversity_type,
                                    fill = diversity_type)) + 
...
fig4c
ggsave(paste0("atherton_figure4c_ITSdiversitybelowground_vs_dfb_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = fig4c, dpi = 300, width = 5.75, height = 3.5, units = "in")
...
fig4c_wlegend <- ggplot(diversity_long, aes(x = log(tree_dist_to_boston_km +1), 
  ...
  scale_fill_manual(values = linear_pal, guide = "none") + 
  theme_classic() + 
...
ggsave(paste0("atherton_figure4c_ITSdiversitybelowground_vs_dfb_wlegend_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = fig4c_wlegend, dpi = 300, width = 5.75, height = 3.5, 
       units = "in")
...
metadata_soil_16s <- metadata_16s[which(metadata_16s$sample_type != "Leaf"),]
...
# Plot figure 4d
fig4d <- ggplot(diversity_long, aes(x = log(tree_dist_to_boston_km +1), 
                                    y = value, color = diversity_type, 
                                    fill = diversity_type)) + 
  geom_point(aes(shape = sample_type), alpha = 0.5, size = 2) +
  geom_smooth(method = "lm", alpha = 0.12, 
...
fig4d
ggsave(paste0("atherton_figure4d_16Sdiversitybelowground_vs_dfb_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = fig4d, dpi = 300, width = 5.75, height = 3.5, units = "in")
