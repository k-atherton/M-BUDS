...
# Transform the data
metadata_soil_16s$soil_no3 <- log(metadata_soil_16s$soil_no3+1)
...
summary_data <- metadata_soil_16s %>%
  group_by(site_edge) %>%
  summarise(
    mean = mean(soil_no3, na.rm = TRUE),
    se = sd(soil_no3, na.rm = TRUE) / sqrt(n()),
    sd = sd(soil_no3, na.rm = TRUE),
    .groups = 'drop'
  )

# Visualize
fig3a <- ggplot(metadata_soil_16s, aes(x = site_edge, y = soil_no3, 
                                       group = 1)) +
  # this adds the jittered points
  geom_jitter(aes(stroke = NA), color = linear_pal[1], 
              size = 0.75,  alpha = 0.2) +
  # this adds the lines that follow the mean across tree types
  stat_summary(fun = mean, geom = "line",
               position = position_dodge(width = 0.75), linetype = "dashed",
               linewidth = 0.8, alpha = 0.5, color = linear_pal[1]) +
  # this adds the error bars
  geom_errorbar(data = summary_data,
                aes(x = site_edge, y = mean, ymin = mean - sd, ymax = mean + sd,
                    group = site_edge), color = linear_pal[1],
  ...
  geom_crossbar(data = summary_data,
                aes(x = site_edge, y = mean, ymin = mean - se, 
                    ymax = mean + se),
                    fill = linear_pal[1], color = linear_pal[1],
  ...
  geom_text(family = "Arial",
            ...
            show.legend = FALSE) +
  theme_classic() + 
  labs (y="log(Soil inorganic NO<sub>3</sub><sup>-</sup> content (ug/g))", x="") +
  ...
  scale_fill_manual(values = linear_pal[1], guide= "none") + 
  scale_color_manual(values = linear_pal[1], guide = "none") + 
