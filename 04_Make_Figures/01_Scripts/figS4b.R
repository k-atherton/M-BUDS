...
length(unique(metadata_osoil_16s$tree_id))
...
pca_osoil_df_16s$tree_age <- "O"
pca_osoil_df_16s$tree_age[which(pca_osoil_df_16s$tree_dbh_2021 < 30)] <- "M"
pca_osoil_df_16s$tree_age[which(pca_osoil_df_16s$tree_dbh_2021 < 10)] <- "Y"

print(adonis2(osoil_aitch_16s~pca_osoil_df_16s$tree_pit_type))
print(adonis2(osoil_aitch_16s~pca_osoil_df_16s$tree_age))
print(adonis2(osoil_aitch_16s~pca_osoil_df_16s$tree_species))

# Plot figure s4b
figs4b_osoil <- ggplot(pca_osoil_df_16s, 
...
figs4b_osoil
ggsave(paste0("atherton_figures4b_16spcaosoil_", 
              format(Sys.Date(), "%Y%m%d"), ".pdf"), device = cairo_pdf, 
       plot = figs4b_osoil, dpi = 300, width = 6, height = 3.5, units = "in")